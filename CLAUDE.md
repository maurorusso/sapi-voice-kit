...
- **Cross-process speech overlap, observed live.** Two parallel Claude Code sessions with this plugin active could speak at the same time — each process owns its own `SpeechSynthesizer` with no knowledge of any other process, so simultaneous speech overlapped on the shared audio device and was unintelligible. Fixed with a named Windows Mutex (`Global\SapiVoiceKitSpeak`) around the actual `Speak()` call in `Invoke-SpeechSynthesis`, shared by every caller — processes now take turns instead of talking over each other. A 60s `WaitOne` timeout keeps a crashed or unusually slow holder from permanently silencing everyone else.
