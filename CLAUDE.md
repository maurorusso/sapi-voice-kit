...
- **Spanish question intonation: investigated, no fix found, documented as a known limitation.** Questions ("¿...?") read with weak or no rising interrogative intonation - confirmed this is NOT something this plugin's code causes: a raw `Speak()` call with zero processing already sounds this way. Tried and rejected: SSML `<prosody pitch="+X%">` over the whole sentence (inconsistent - helped some questions, made others sound like statements), pitch boost scoped to only the final word(s) (same inconsistency), and switching to a modern OneCore voice (`Windows.Media.SpeechSynthesis`, confirmed reachable from Windows PowerShell 5.1 and audibly different from the SAPI5 Desktop voice) - Pablo (OneCore) showed the identical lack of question intonation as Helena Desktop, side by side, same text. Conclusion: neither legacy SAPI5 nor modern OneCore Microsoft Spanish voices reliably produce natural interrogative prosody - this is a real limitation of "100% native Windows TTS, no API keys," not a bug to chase further. A neural TTS engine (ElevenLabs, Azure Neural, etc.) would likely handle this correctly, at the cost of the API-key/external-dependency tradeoff this project exists specifically to avoid.
- **The /ʃ/ ("sh") phoneme isn't well supported by this voice - "push" reads as "pus," and that's the best available option.** Confirmed live across four phoneme spellings (`pʊʃ`, the affricate `pʊtʃ`, without the short u, and a doubled `ʃʃ`) - all four came out sounding identical, all missing the "sh" sound. This isn't a spelling problem: this voice's phone inventory appears to fall back to a plain /s/ for this IPA symbol regardless of how it's written. Confirmed the current entry is still worth keeping anyway: the same word with no pronunciation hint at all sounds worse, not better. `function` uses the same phoneme mid-word and likely has the same limitation - not worth a special case for one imperfect-but-still-better-than-nothing sound.
